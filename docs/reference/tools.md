...
Project delivery reports — per-assignee workload, portfolio-wide project health, and a daily stand-up digest. Same composition style as the reports above (filters, thresholds, `timezone_offset`); see the docstrings in `src/odoo_pulse/services/projects/` and `src/odoo_pulse/tools/reports/workflows.py` for the full argument list.
