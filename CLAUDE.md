...
- `tools_reports_ops.py` — operations report tools (`procurement_watch`, `production_health`): purchasing and manufacturing health. Same `build_report` envelope and composition style; loaded as part of the `reports` group alongside `tools_workflows` and `tools_reports`.
