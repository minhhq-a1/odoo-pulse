...
This is an MCP server that exposes Odoo's XML-RPC external API as MCP tools. The entry point is `src/odoo_pulse/server.py`, which imports all tool modules as side effects — each import registers `@mcp.tool()` (and, for `resources.py`, `@mcp.resource()`) functions with the shared FastMCP instance.
...
- `src/odoo_pulse/resources.py` — the `odoo://{model}/{id}` MCP resource template (the only MCP Resource; everything else is a tool): one live record as JSON with all stored fields, `safe()`-wrapped like every tool; not-found returns the `{"error": ...}` envelope. Loaded as part of the `core` group.
...
- `src/odoo_pulse/project_shared.py` — non-tool shared helpers for the project-status family: budget-model abstraction (`_budget_sources`, `_budget_by_project`, moved from `tools_reports_projects`), `derive_project_health` (THE single source of truth for derived health verdicts), `analytic_money` (positive-cost convention), `paged_search_read` (server-side offset pagination), `periods_domain` (OR-of-ranges domains), and the sub-task hour helpers (`fetch_subtasks`/`sum_hours`/`subtasks_by_month`).
- `src/odoo_pulse/tools_project_detail.py` — dashboard-backing tools (`project_subtask_hours`, `project_dashboard`, `portfolio_health`) for the "Project Status" artifact. Free-form output schema per the spec (NOT the `build_report` envelope — intentional deviation; these feed a dashboard, not a reader). Sections of `project_dashboard` run sequentially with per-section soft-fail (`errors` key). Loaded in the `reports` group.
- `src/odoo_pulse/tool_groups.py` — maps `ODOO_TOOL_GROUPS` (default `core,reports`) to the tool/resource modules `src/odoo_pulse/server.py` imports; unknown group names fail at startup.
