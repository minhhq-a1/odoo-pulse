...
- `tools_workflows.py` — composed, opinionated workflow tools that answer a business question in one call (e.g. `sprint_health`). Read-only; compose `search_read`/aggregates server-side and return the `build_report` envelope.
