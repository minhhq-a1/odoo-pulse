...
def overdue_project_subtask_count(
    context: Any,
    *,
    project_id: int | None = None,
    project_name: str | None = None,
) -> int:
    domain: list = [("parent_id", "!=", False), ("date_deadline", "<", str(context.today))]
    if project_id is not None:
        domain.append(("project_id", "=", project_id))
    if project_name is not None:
        domain.append(("project_id.name", "=", project_name))
    return context.client.search_count("project.task", domain)


def overdue_subtask_count_by_project(
    context: Any,
    *,
    ...
    if project_names:
        domain.append(("project_id.name", "in", project_names))
    rows = paged_search_read(
        context.client, "project.task", domain, fields=["project_id"]
    )
    counts: dict[Any, int] = {}
    for r in rows:
        p = r.get("project_id")
        if not p:
            continue
        key = p[0] if project_ids else p[1]
