"""Absence overview report service."""
...
from .metrics import approved_leave_domain


def build_absence_overview(
    client,
    *,
    days: int = 14,
    ...
    def leave_lists():
        ...
        approved = fetch_with_truncation(
            ...
            approved_leave_domain(
                context,
                starts_before=utc_bound(
                    horizon + timedelta(days=1), timezone_offset
                ),
                ends_at_or_after=utc_bound(today, timezone_offset),
            ),
