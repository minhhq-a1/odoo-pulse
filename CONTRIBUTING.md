...
Tests inject a `FakeClient` into `runtime._client` (see `tests/conftest.py`).
...
3. Report-style tools return the `build_report` envelope
   (`workflow_helpers.py`) — numbers + `highlights` + `risks` + a verdict.
4. New modules must be added to a group in `tool_groups.GROUP_MODULES`.
...
- Read tools must stay read-only; anything that writes goes through the
  four-layer guard in `odoo_client._check_write` and returns a dry-run
